...
class PetRemoteDataSourceImpl implements PetRemoteDataSource {
  ...
  Map<String, dynamic> _getFallbackBreed(bool isDog) {
    final random = Random();
    ...
    final catBreeds = [
      ...
      {'id': 3, 'name': 'Persian', 'temperament': 'Quiet, Sweet, Gentle'},
      {
        ...
        'temperament': 'Calm, Dignified, Easygoing'
      },
      {
        'id': 5,
        'name': 'Ragdoll',
    ...
    final breeds = isDog ? dogBreeds : catBreeds;
    return breeds[random.nextInt(breeds.length)];
  }

  Future<Map<String, dynamic>> _fetchRandomBreed({required bool isDog}) async {
    // For web compatibility, skip external API calls and use fallback data directly
    if (kIsWeb) {
      return _getFallbackBreed(isDog);
    }
  ...
  Future<List<String>> _fetchPetImages(
    ...
    // For web compatibility, skip external API calls and use placeholder images
    if (kIsWeb) {
